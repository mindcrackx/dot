...
#
# dot/bash/core.sh — portabler, maschinen-agnostischer Shell-Kern.
# Wird von ~/.bashrc (dem Loader) gesourct, danach das aktive Profil
# (profiles/server.sh oder profiles/desktop.sh) und ~/.bash_personal.
# Regel: nichts hier darf auf einem headless-Server brechen -> Aliases/
# Seiteneffekte immer per `_have <tool> &&` absichern. Desktop-spezifisches
# (X/Wayland-Tastatur o.ae.) gehoert nach profiles/desktop.sh.
...
export DOTFILES="${DOT_DIR:-$GHREPOS/dot}"  # DOT_DIR wird vom Loader gesetzt
...
# Editor dynamisch: erstes vorhandenes nehmen (mgmt hat z.B. kein vi/vim)
for _e in nvim vim vi nano; do
  _have "$_e" && { export EDITOR="$_e" VISUAL="$_e" EDITOR_PREFIX="$_e"; break; }
done; unset _e
...
__ps1() {
  ...
  kctx=$(_have kubectl && kubectl config current-context 2>/dev/null)
...
# Portabilitaets-Regel: Aliase, die ein reales Kommando SHADOWEN oder ein
# nicht-coreutil Tool brauchen, immer per `_have` absichern — sonst geht
# z.B. `grep` auf einem Server ohne pcregrep kaputt.

unalias -a
# immer verfuegbar (coreutils / reine Navigation)
...
_have google   && alias '??'='google'
_have bing     && alias '???'='bing'
_have clip     && alias coin="clip '(yes|no)'"
_have live     && alias iam='live'
_have neo      && alias neo='neo -D -c gold'

_have vim && alias vi=vim
_have vim && alias view='vim -R'   # nur sinnvoll, wenn ein vi(m) existiert
...
# IaC-Completions (Pfad dynamisch — tofu ersetzt terraform im Homelab)
_have terraform && complete -C "$(command -v terraform)" terraform
_have terraform && complete -C "$(command -v terraform)" tf
_have tofu      && complete -C "$(command -v tofu)" tofu
_have tofu      && complete -C "$(command -v tofu)" tf

# Hinweis: Profil (server/desktop) und ~/.bash_personal werden vom Loader
# (~/.bashrc) NACH dieser Datei gesourct — hier bewusst nichts davon.
